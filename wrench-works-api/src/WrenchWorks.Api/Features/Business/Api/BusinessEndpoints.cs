...
public static class BusinessEndpoints
{
    public static void Map(IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/",
            (IBusinessEndpointHandler handler, CancellationToken ct) =>
                handler.GetAsync(ct));

        group.MapPut("/",
            (UpdateBusinessRequest request, IBusinessEndpointHandler handler, CancellationToken ct) =>
                handler.UpdateAsync(request, ct))
