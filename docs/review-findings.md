...
| 19 | `RemovePartAsync` queried the unfiltered `JobPartLines` *before* the tenant check on the parent job — backwards from `RemoveLaborAsync`. Not a leak (the parent check still blocked it) but an existence oracle: foreign job → 500, own missing line → 404. **FIXED 1 Sep 2026** — the repository split put the two methods side by side and the asymmetry became obvious; `JobService.RemovePartAsync` now resolves the parent job first, so a foreign job is a 404 like every other slice | Verified · fixed |
